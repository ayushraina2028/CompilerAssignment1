...
namespace {
struct HoistAnticipatedExpressions
    : public PassInfoMixin<HoistAnticipatedExpressions> {
  using Expression = tuple<unsigned int, Value *, Value *>;
  unsigned int HoistCounter = 0;
  DenseMap<Expression, Value *> HoistedExpressions;

  void printSets(Function &F, DenseMap<BasicBlock *, BitVector> &Gen,
  ...
  PreservedAnalyses run(Function &F, FunctionAnalysisManager &AM) {
    ...
    do {
      ...
      for (auto &BB : F) {
        for (auto &Inst : BB) {
          if (auto *BinOp = dyn_cast<BinaryOperator>(&Inst)) {
            ...
            Expression exprKey = make_tuple(
                Instruction::CastOps(Cast->getOpcode()), op, nullptr);
            ...
            if (Call->getCalledFunction() &&
              ...
              if (ExpressionToBitMap.find(exprKey) ==
                  ExpressionToBitMap.end()) {
                outs() << "  Adding new expression at index " << nextBitIndex
                       << "\n";
                ExpressionToBitMap[exprKey] = nextBitIndex++;
              }
      ...
      do {
        ...
        for (auto &BB : F) {
          ...
          if (succ_begin(&BB) != succ_end(&BB)) {
            OUT[&BB] =
                BitVector(numExpressions, true); // Start with universal set
      ...
      if (anticipatedBefore > 0) {
        ...
        OuterChanged = (replacementsMade >
                        0); // Only continue if we actually made replacements
      }
    ...
    outs() << "\n=== Optimization completed after " << iteration
           << " iterations ===\n";
  ...
  int hoistAnticipatedExpressions(
      Function &F, DenseMap<BasicBlock *, BitVector> &OUT,
      DenseMap<Expression, unsigned int> &ExpressionToBitMap,
      DominatorTree &DT) {
        ...
        if (opcode == Instruction::SRem) {
          ...
          if (opcode == Instruction::SIToFP) {
            newInst = Builder.CreateSIToFP(
                op1, Type::getDoubleTy(F.getContext()), hoistedName);
          } else if (opcode == Instruction::FPToSI) {
            newInst = Builder.CreateFPToSI(
                op1, Type::getInt32Ty(F.getContext()), hoistedName);
          } else {
            newInst = Builder.CreateUIToFP(
                op1, Type::getDoubleTy(F.getContext()), hoistedName);
          }
            ...
            if (auto *binOp = dyn_cast<BinaryOperator>(&I)) {
              ...
              if (binOp->getOpcode() == opcode &&
                  ((curOp1 == op1 && curOp2 == op2) ||
                   (binOp->isCommutative() && curOp1 == op2 &&
                    curOp2 == op1))) {
